...
```{r}
##Subset
P46_cc <- subset(P46_Mouse, idents = c('0 Basal Krtno', '2 Basal Krtno', '3 Basal Krtno', '6 Fibro', '8 DP', '9 Fibro', 
                     '10 LBV', '11 Basal Krtno', '14 DP', 
                     '17 Div Krtno', '18 Fibro', 
                     '20 Pericyte', '21 Fascia', '24 Fibro', 
                     '25 BV'), invert = FALSE)#subset the fibroblast supercluster
P46_cc#view the subset

##Rename clusters to their #'s since this cleans up the CellChat plots
cluster_names <- c('BK', '2', '3', '6', '8', '9', 
                     '10', '11', '14', 
                     '17', '18', 
                     '20', '21', '24', 
                     '25')
...
names(new.cluster.ids) <- levels(P46_cc)
P46_cc <- RenameIdents(P46_cc, new.cluster.ids)
```

#Initialize cellchat object and db
```{r}
data.input <- GetAssayData(P46_cc, assay = "SCT", slot = "data") # normalized data matrix
labels <- Idents(P46_cc)
...
```{r}
save(cellchat, file = '~/Desktop/Sean_CellChat/Object/Liu2022_P46_Mouse_cellchat.Rdata')
```
...
path_out <- '~/Desktop/Sean_CellChat/Plots/P46_EpiDer/Cluster_CirclePlot/'
setwd(path_out)#need to change working directory to export plots b/c the plot is not a ggplot object
umap_cols <- c('#16539e', '#4d8ad5', '#0c3695', '#002783', '#cc0000',
                     ...
                     '#8c6bb1', '#3b7bca', '#f16913', '#DD0000', '#640001')#string vector of my custom colors for UMAP
cols_i <- as.numeric(levels(P46_cc))#get cluster number
cols_i[1] <- 0#since cellchat can't take 0 as a cluster id, now need to replace 'O' --> NA from as.numeric w/ 0 for i
...
path_out <- '~/Desktop/Sean_CellChat/Plots/P46_EpiDer/Path_CirclePlot/'
setwd(path_out)#need to change working directory to export plots b/c the plot is not a ggplot object
...
path_out <- '~/Desktop/Sean_CellChat/Plots/P46_EpiDer/LR_Pair_CirclePlot/'
setwd(path_out)#need to change working directory to export plots b/c the plot is not a ggplot object
