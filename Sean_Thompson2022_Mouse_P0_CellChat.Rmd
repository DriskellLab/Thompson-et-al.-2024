...
```{r}
##Subset
P0_cc <- subset(scRNA_WT2, idents = c('0 PF', '1 RF', '2 PF', '3 RF', '4 DP', 
                     '5 Div Fibro', '6 Keratinocyte', '7 Pericyte', 
                     '8 BV', '14 Pre-adipocyte', '17 BV', '19 DS'), invert = FALSE)#subset the fibroblast supercluster
P0_cc#view the subset

##Rename clusters to their #'s since this cleans up the CellChat plots
cluster_names <- c('PF', '1', '2', '3', '4', 
                     '5', '6', '7', 
                     '8', '14', '17', '19')
...
names(new.cluster.ids) <- levels(P0_cc)
P0_cc <- RenameIdents(P0_cc, new.cluster.ids)
```

#Initialize cellchat object and db
```{r}
data.input <- GetAssayData(P0_cc, assay = "SCT", slot = "data") # normalized data matrix
labels <- Idents(P0_cc)
...
```{r}
save(cellchat, file = '~/Desktop/Sean_CellChat/Object/Thompson2022_P0_Mouse_cellchat.Rdata')
```
...
path_out <- '~/Desktop/Sean_CellChat/Plots/P0_EpiDer/Cluster_CirclePlot/'
setwd(path_out)#need to change working directory to export plots b/c the plot is not a ggplot object
umap_cols <- c('#238b45', '#74c476', '#006d2c', '#a1d99b', '#00441b', '#41ab5d', '#225ea8', '#7fcdbb',
                       '#8c6bb1', '#ef3b2c', '#a50f15', '#fc8d59', '#ffffff', '#cb181d', '#bdbdbd', '#f768a1', '#800026',
                       '#88419d', '#ae017e', '#78c679', '#f16913')#string vector of my custom colors for UMAP
cols_i <- as.numeric(levels(P0_cc))
...
path_out <- '~/Desktop/Sean_CellChat/Plots/P0_EpiDer/Path_CirclePlot/'
setwd(path_out)#need to change working directory to export plots b/c the plot is not a ggplot object
...
path_out <- '~/Desktop/Sean_CellChat/Plots/P0_EpiDer/LR_Pair_CirclePlot/'
setwd(path_out)#need to change working directory to export plots b/c the plot is not a ggplot object
