...
```{r}
##Subset
DimPlot(scRNA_SoleBoldo)
SoleBoldo_cc <- subset(scRNA_SoleBoldo, idents = c('1 Fibroblast', '2 Fibroblast', '4 Fibroblast',
                     '5 Blood Vessel', '6 Basal Krtno', '7 Upper Krtno', '8 Pericyte', 
                     '10 Fibroblast', '12 Pericyte', '13 LBV',
                     '17 Blood Vessel', '18 Krtno', '20 APM'), invert = FALSE)#subset
SoleBoldo_cc#view the subset

##Rename clusters to their #'s since this cleans up the CellChat plots
cluster_names <- c('1', '2', '4',
                     '5', '6', '7', '8', 
                     '10', '12', '13',
                     '17', '18', '20')
...
names(new.cluster.ids) <- levels(SoleBoldo_cc)
SoleBoldo_cc <- RenameIdents(SoleBoldo_cc, new.cluster.ids)
```

#Initialize cellchat object and db
```{r}
data.input <- GetAssayData(SoleBoldo_cc, assay = "SCT", slot = "data") # normalized data matrix
labels <- Idents(SoleBoldo_cc)
...
```{r}
save(cellchat, file = '~/Desktop/Sean_CellChat/Object/SoleBoldo_cellchat_v2.Rdata')
```
...
path_out <- '~/Desktop/Sean_CellChat/Plots/SoleBoldo_EpiDer/Cluster_CirclePlot/'
setwd(path_out)#need to change working directory to export plots b/c the plot is not a ggplot object
umap_cols <- c('#900101', '#238b45', '#006d2c', '#640001', '#41ab5d',
                     '#8c6bb1', '#0c3695', '#225ea8', '#7fcdbb', '#790001', 
                     '#00441b', '#AD0000', '#4db89f', '#88419d', '#fc8d59', 
                     '#BB0000', '#cc0000', '#7850a7', '#4572aa', '#DD0000', '#f768a1')#string vector of my custom colors for UMAP
cols_i <- as.numeric(levels(SoleBoldo_cc))
...
path_out <- '~/Desktop/Sean_CellChat/Plots/SoleBoldo_EpiDer/Path_CirclePlot/'
setwd(path_out)#need to change working directory to export plots b/c the plot is not a ggplot object
...
par(mfrow=c(1,1))
n_loop <- 1#storage variable to iterate through
while (n_loop <= length(pathways.show[1:31])) {
...
while (n_loop <= length(pathways.show[33:length(pathways.show)])) {
  #print(pathways.show[n_loop])#test
  print(netVisual_heatmap(cellchat, signaling = pathways.show[n_loop], color.heatmap = "Reds"))#want to view one pathway at a time
  n_loop <- n_loop + 1#iterate to next pathway
}
...
path_out <- '~/Desktop/Sean_CellChat/Plots/SoleBoldo_EpiDer/LR_Pair_CirclePlot/'
setwd(path_out)#need to change working directory to export plots b/c the plot is not a ggplot object
