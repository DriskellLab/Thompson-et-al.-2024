...
##Subset
DimPlot(Glover2023)
Glover_cc <- subset(Glover2023, idents = c('0', '1', '2', '3', '4', '6', '7', '8', '12'), invert = FALSE)#subset
Glover_cc#view the subset

##Rename clusters to their #'s since this cleans up the CellChat plots
cluster_names <- c('PF', '1', '2', '3', '4', '6', '7', '8', '12')#replace '0' id w/ something else since CellChat can't have '0' as an ID

new.cluster.ids <- cluster_names
names(new.cluster.ids) <- levels(Glover_cc)
Glover_cc <- RenameIdents(Glover_cc, new.cluster.ids)
```

#Initialize cellchat object and db
```{r}
data.input <- GetAssayData(Glover_cc, assay = "SCT", slot = "data") # normalized data matrix
labels <- Idents(Glover_cc)
...
```{r}
save(cellchat, file = '/t2tb/Object/Glover2023_cellchat.Rdata')
```
...
path_out <- '~/Desktop/Sean_CellChat/Plots/Glover_EpiDer/Cluster_CirclePlot/'
setwd(path_out)#need to change working directory to export plots b/c the plot is not a ggplot object
umap_cols <- c('#00441b', '#0c3695', '#225ea8', '#41ab5d', '#7fcdbb', 
                       '#ffab4b', '#006d2c', '#238b45', '#8c6bb1', '#640001', 
                       '#fc8d59', '#BB0000', '#88419d')#string vector of my custom colors for UMAP
cols_i <- as.numeric(levels(Glover_cc))
...
path_out <- '~/Desktop/Sean_CellChat/Plots/Glover_EpiDer/Path_CirclePlot/'
setwd(path_out)#need to change working directory to export plots b/c the plot is not a ggplot object
...
path_out <- '~/Desktop/Sean_CellChat/Plots/Glover_EpiDer/LR_Pair_CirclePlot/'
setwd(path_out)#need to change working directory to export plots b/c the plot is not a ggplot object
