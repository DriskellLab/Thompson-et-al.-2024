...
```{r}
##Subset
D10_cc <- subset(D10, idents = c('3 Rete Krtno', '4 Krtno',
                     '5 RF', '6 Krtno', '7 Krtno', '8 Div Krtno', '9 Pericyte',
                     '10 Blood Vessel', '11 PF', '14 Fibro', 
                     '15 ACTA2 Fibro', '17 Lymphatic Vessel'), invert = FALSE)#subset the fibroblast supercluster
D10_cc#view the subset

##Rename clusters to their #'s since this cleans up the CellChat plots
cluster_names <- levels(D10_cc)
cluster_names <- as.data.frame(strsplit2(x = cluster_names, split = ' '))

new.cluster.ids <- cluster_names$V1
...
```{r}
save(cellchat, file = '~/Desktop/Sean_CellChat/Object/p10_cellchat_v3.Rdata')
```
...
path_out <- '~/Desktop/Sean_CellChat/Plots/P10_EpiDer/Cluster_CirclePlot/'
...
cols_i <- as.numeric(levels(D10_cc))
...
path_out <- '~/Desktop/Sean_CellChat/Plots/P10_EpiDer/Path_CirclePlot/'
setwd(path_out)#need to change working directory to export plots b/c the plot is not a ggplot object
...
par(mfrow=c(1,1))
n_loop <- 1#storage variable to iterate through
while (n_loop <= length(pathways.show[1:44])) {
...
path_out <- '~/Desktop/Sean_CellChat/Plots/P10_EpiDer/LR_Pair_CirclePlot/'
setwd(path_out)#need to change working directory to export plots b/c the plot is not a ggplot object
